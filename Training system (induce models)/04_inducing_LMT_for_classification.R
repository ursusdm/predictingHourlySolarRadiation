...
examples_predicted_kd <- mutate_at(examples_predicted_kd, vars(Today_cluster14_experts), as.factor)
...
if(TESTING)
{
  ...
  calculate_errors_given_centroids <- function(predicted_centroids)
  {
    ...
    demo_dataset <- read.csv("demo_dataset.csv")
    # select ghext and gh information
    ghext <- demo_dataset %>% select(ghext9,ghext10,ghext11,ghext12,ghext13,ghext14,ghext15,ghext16)
    gh    <- demo_dataset %>% select(gh9,gh10,gh11,gh12,gh13,gh14,gh15,gh16)
